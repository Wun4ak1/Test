...
async def handle_edit_order_menu(callback: CallbackQuery, state: FSMContext):
    ...
    if not order:
        await callback.message.answer("Сизнинг фаол буюртмангиз йўқ, {user_type}.")
        return
...
async def edit_date(cb: CallbackQuery, state: FSMContext):
    ...
    if day_key == "custom":
        await cb.message.answer("📅 Сана киритинг (Йил-Ой-Кун, масалан: 2025-04-20):")
        await state.update_data(user_type=user_type)
...
async def save_custom_date(msg: Message, state: FSMContext):
    try:
        ...
        await msg.answer("❌ Формат нотўғри. Йил-Ой-Кун тарзда киритинг, масалан 2025-04-25.")
        return
